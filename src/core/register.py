...
class RegistrationEngine:
    ...
    def __init__(
        ...
        # 浏览器指纹 (从 any-auto-register 移植)
        self._init_browser_fingerprint()

    def _init_browser_fingerprint(self):
        """初始化浏览器指纹"""
        self.ua, self.chrome_major, self.chrome_full, self.sec_ch_ua = openai_utils._random_chrome_version()
        self.device_id = openai_utils.generate_device_id()
        self.accept_language = "en-US,en;q=0.9"

    def _build_headers(self, url: str, accept: str, referer: str = None, 
                       origin: str = None, content_type: str = None,
                       fetch_site: str = "same-origin") -> dict:
        """构建浏览器请求头"""
        return openai_utils.build_browser_headers(
            url=url,
            user_agent=self.ua,
    ...
    def _submit_auth_start(
        ...
        *,
        ...
        """
        try:
            signup_url = OPENAI_API_ENDPOINTS["signup"]
            ...
            # 添加 DataDog 追踪头
            headers.update(openai_utils.generate_datadog_trace())
            # 添加设备 ID
    ...
    def _submit_login_password(self) -> SignupFormResult:
        """提交登录密码，进入邮箱验证码页面。"""
        try:
            pwd_url = OPENAI_API_ENDPOINTS["password_verify"]
            headers = self._build_headers(
                url=pwd_url,
                accept="application/json",
    ...
    def _register_password(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # 构建请求头 (添加 DataDog 追踪和 oai-device-id)
            register_url = OPENAI_API_ENDPOINTS["register"]
            headers = self._build_headers(
                url=register_url,
                accept="application/json",
            ...
            sen_token = self.http_client.check_sentinel(self.device_id, flow="username_password_create")
            if sen_token:
                self._log(f"密码注册 sentinel token 获取成功")
                sentinel = json.dumps({
                    "p": "",
                    "t": "",
                    "c": sen_token,
    ...
    def _validate_verification_code(self, code: str) -> bool:
        """验证验证码"""
        try:
            otp_url = OPENAI_API_ENDPOINTS["validate_otp"]
            headers = self._build_headers(
                url=otp_url,
                accept="application/json",
    ...
    def _create_user_account(self) -> bool:
        ...
        try:
            ...
            ca_url = OPENAI_API_ENDPOINTS["create_account"]
            headers = self._build_headers(
                url=ca_url,
                accept="application/json",
            ...
            # 获取 oauth_create_account 流程的 sentinel token
            sen_token = self.http_client.check_sentinel(self.device_id, flow="oauth_create_account")
            if sen_token:
                self._log(f"创建账户 sentinel token 获取成功")
                sentinel = json.dumps({
                    "p": "",
                    "t": "",
                    "c": sen_token,
