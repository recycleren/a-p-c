...
function createElement(tagName) {
  ...
  // Special handling for iframe: needs contentWindow with style
  if (tag === "iframe") {
    const iframeStyle = {};
    const iframeContentWindow = { document: { write: () => {}, close: () => {} }, style: iframeStyle };
    const el = {
      nodeType: 1,
      tagName: "IFRAME",
      nodeName: "IFRAME",
      ...
      setAttribute(key, val) {
        if (String(key).toLowerCase() === "src") this.src = String(val);
      },
      getAttribute() {
        return null;
      },
    ...
    iframeContentWindow.window = iframeContentWindow;
    iframeContentWindow.location = { href: "", replace: () => {} };
    return el;
  }
  return {
    nodeType: 1,
