...
class OpenAIHTTPClient(HTTPClient):
    ...
    def check_ip_location(self):
        ...
        """
        # 国内直连这些地址会被墙，逐个尝试直到成功
        # 注意：必须强制走代理，因为 self.proxies 可能没正确传入
        check_urls = [
            "https://1.1.1.1/cdn-cgi/trace",   # Cloudflare 主用
            "https://cp.cloudflare.com/cdn-cgi/trace",  # Cloudflare 备用
            "https://www.cloudflare.com/cdn-cgi/trace",  # Cloudflare 备用2
        ]
        for url in check_urls:
            try:
                response = self.get(url, timeout=8)
                trace_text = response.text
        ...
        # 所有地址都失败，尝试直接用代理出口 IP
        try:
            import socket
            # 从代理信息中提取 IP（如果有的话）
            # 如果有代理，从 OpenAI API 的响应头看出口 IP
            response = self.get("https://api.openai.com/v1/models", timeout=8)
            # API 本身返回 401 没关系，主要是看出口 IP
            # 通过代理访问 OpenAI 成功说明代理是通的
            logger.info(f"✅ 通过代理访问 OpenAI API 成功（IP 可用）")
            return True, "PROXY_OK"
        except Exception as e:
            logger.error(f"❌ 检查 IP 地理位置失败（所有检测点均超时，代理可能不通）: {e}")
            return False, None
    ...
    def check_sentinel(self, did: str, flow: str = "authorize_continue", proxies: Optional[Dict] = None) -> Optional[str]:
        ...
        Args:
            did: Device ID
            flow: Sentinel flow 类型 (authorize_continue, username_password_create, oauth_create_account)
            proxies: 代理配置
        ...
        # username_password_create / oauth_create_account 需要完整 PoW 解（QuickJS），不能用简单 HTTP PoW
        _QUICKJS_FLOWS = {"username_password_create", "oauth_create_account"}

        if flow in _QUICKJS_FLOWS:
            try:
                from .sentinel_quickjs import get_sentinel_token_via_quickjs
                return get_sentinel_token_via_quickjs(
                    flow=flow,
                    proxy=self.proxies,
                    timeout_ms=45000,
                    device_id=did,
                    log_fn=lambda msg: logger.info(f"[QuickJS Sentinel] {msg}"),
                )
            except Exception as e:
                logger.error(f"Sentinel QuickJS 失败: {e}，回退到 HTTP PoW")
                # 回退到 HTTP PoW（可能仍然 400，但至少尝试一下）
